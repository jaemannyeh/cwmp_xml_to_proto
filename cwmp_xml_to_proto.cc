...
//
// ./cwmp_xml_to_proto tr-069-1-0-0-full.xml
    ...
    if (xmlStrcmp(node->name, (const xmlChar *)"description")==0) { 
      ...
      if (node->parent && xmlStrcmp(node->parent->name,(const xmlChar *)"parameter")==0) {
        ...
        if (messages[indent].empty()) {
          proto << "//"; // If paramaeter_name has no object_name, comment the parameter out.  
        }
        proto << left_margin(indent+1) << std::left << setw(10) << get_scalar_value_type(node)
        ...
        if (prev_object_name != object_name) {
          ...
          proto << left_margin(indent) << "message " << message << " { // " << object_name << endl;
...
static int cwmp_xml_to_proto(char *cwmp_xml_file_name, string& package_name) {
  ...
  proto_name += ".proto";
  proto.open(proto_name);

  proto << "// " << proto_name << endl;
  proto << "// Generated by " << __FILE__ << endl;
  proto << "// source: " << cwmp_xml_file_name << endl;
  proto << endl;
